...
|---|--------|--------|-------|---------|
| 42 | **Precise vertex-rewrite tracking** (`XEMU_VTX_PRECISE`, off by default) | guard fires on any page *uploaded* | conflicting pages 10.76 -> 0.15/frame (-99%), confirming most stalls were pages no draw reads; but p99 +43%, 1% low -41%, and `vertex_buffer_dirty` was only 0.36 ms in this scene | NOT PROVEN - mechanism works, no headroom to win here; needs a scene where the stall actually costs |
