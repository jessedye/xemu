...
void pgraph_vk_update_descriptor_sets(PGRAPHState *pg)
{
    ...
    for (int i = 0; i < NV2A_MAX_TEXTURES; i++) {
        image_infos[i] = (VkDescriptorImageInfo){
            .imageLayout = r->texture_bindings[i]->alias_surface ?
...
static void update_shader_uniforms(PGRAPHState *pg)
{
    ...
    /* A different binding owns a different allocation, so its contents matching
     * what it last held says nothing about what was uploaded. */
